...
Product Description: ${cleanedProductDescription}`;
  ...
  const prompt = `You are a professional copywriter. Your task is to write engaging and lead generating PAS (Problem, Agitate, Solution) sales copy for the given product. Highlight the problem, agitate (exaggerate the problem), and then provide a solution in a persuasive manner.
...
# Problem:`;
  ...
  for (let i = 0; i < numberOfSuggestions; i++) {
    const pasSalesCopy = await generateContentUsingGPT4('gpt-4oo', 150, prompt, 1, 0, 0, ['\n\n', 'Product Description:']);
...
Product Description: ${cleanedProductDescription}`;
  ...
  const prompt = `You are a professional copywriter. Your task is to write engaging and lead generating PASO (Problem, Agitate, Solution, Outcome) sales copy for the given product. Highlight the problem, agitate (exaggerate the problem), provide a solution, and describe the positive outcome in a persuasive manner.
...
# Problem:`;
  ...
  for (let i = 0; i < numberOfSuggestions; i++) {
    const pasoSalesCopy = await generateContentUsingGPT4('gpt-4oo', 200, prompt, 1, 0, 0, ['\n\n', 'Product Description:']);
...
Product Description: ${cleanedProductDescription}`;
  ...
  const prompt = `You are a professional copywriter. Your task is to write engaging and lead generating AIDA (Attention, Interest, Desire, Action) sales copy for the given product. Start with grabbing attention, then build interest, create desire, and end with a call to action.
...
# Attention:`;
  ...
  for (let i = 0; i < numberOfSuggestions; i++) {
    const aidaSalesCopy = await generateContentUsingGPT4('gpt-4oo', 200, prompt, 1, 0, 0, ['\n\n', 'Product Description:']);
